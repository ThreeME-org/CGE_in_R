...
a_3ME.append py_n * y = c_y * y * (1.0 + m_up )
a_3ME.append d(log(py_e)) = adjust_pe_pe1y * d(log(py_e(-1))) + adjust_pe_p1y * d(log(py(-1))) + adjust_pe_pny * d(log(py_n)) 
a_3ME.append y + m  = ch + i + g + x 
a_3ME.append p * (y + m ) = py * y + pm * m 
a_3ME.append x = wd * (py / pw) ^ (-rho_x)
...
a_3ME.append sav_row = pm * m - py * x
...
a_3ME.append prof = py * y - w * l - c_k * k
a_3ME.append m = (y + m ) * phi_m * (pm / p) ^ (-rho_m)
...
a_3ME.append log(py) = adjust_p_pny * log(py_n) + (1.0 - adjust_p_pny) * (log(py(-1)) + d(log(py_e)) ) 
